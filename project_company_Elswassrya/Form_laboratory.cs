...
{
    public partial class Form_laboratory : Form
    {
        public Form_laboratory()
        {
            InitializeComponent();
        }
        ...
        private void Form_laboratory_Load(object sender, EventArgs e)
        {
        ...
        private void Search()
        {
            try
            {
                ...
                if (comboBox1.SelectedItem.Equals("User Name"))
                {
                    string StrSearch = "SELECT * FROM `track_laboratory` WHERE `UserName_TL`  Like '" + textBox1.Text + "%'";
                    cmd.CommandText = StrSearch;
                    adap = new MySqlDataAdapter();
                    adap.SelectCommand = cmd;
                    DataSet custDB = new DataSet();
                    custDB.Clear();
                    adap.Fill(custDB, "Track_Laboratory");
                    CrystalReport_Laboratory myReport = new CrystalReport_Laboratory();
        ...
        {
            string StrSearch = "SELECT * FROM `track_laboratory`";
            ...
            if (comboBox1.SelectedItem.Equals("Date"))
            {
                ...
                StrSearch += "  WHERE `Date_TL` Like '%" + Query + "%'";
                cmd.CommandText = StrSearch;
                adap = new MySqlDataAdapter();
                adap.SelectCommand = cmd;
                DataSet custDB = new DataSet();
            ...
            else if (comboBox1.SelectedItem.Equals("From Date To Date"))
            {
                ...
                StrSearch += "  WHERE `Date_TL`  BETWEEN '" + Query1 + "' AND '" + Query2 + "'";
                cmd.CommandText = StrSearch;
                adap = new MySqlDataAdapter();
                adap.SelectCommand = cmd;
                DataSet custDB = new DataSet();
